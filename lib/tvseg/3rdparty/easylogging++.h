...
///////////////////////////////////////////////////////////////////////////////////
// NOTE 04/2014: Small adaptations made by Nikolaus Demmel; Same license;
//
//  * Disable C++11 on clang (not working when using libstdc++);
//  * Fallback to boost::thread
//  * Add per logger dispatch callbacks
...
#if defined(__GNUC__) && (!defined(__clang__)) && (!_ELPP_NDK) && (_ELPP_CXX0X || _ELPP_CXX11)
...
public:
    Constants (void) :
        ...
        //
        LOG_INFO_LEVEL_VALUE   ("INF") ,
        LOG_DEBUG_LEVEL_VALUE  ("DBG"),
        LOG_WARNING_LEVEL_VALUE("WRN"),
        LOG_ERROR_LEVEL_VALUE  ("ERR"),
        LOG_FATAL_LEVEL_VALUE  ("FAT"),
        LOG_VERBOSE_LEVEL_VALUE("VER"),
        LOG_QA_LEVEL_VALUE     ("QA"),
        LOG_TRACE_LEVEL_VALUE  ("TRC"),
        //
        // Format specifiers
        //
...
#   else
    // NOTE: Thread-safety: assume we already have a lock at this point,
    //       so no synchronization for the static variables needed
    boost::thread::id handle = boost::this_thread::get_id();
    static int nextIndex = 0;
    static std::map<boost::thread::id, int> ids;
    std::map<boost::thread::id, int>::iterator it = ids.find(handle);
    int printId;
    if (it == ids.end()) {
        printId = ids[handle] = nextIndex++;
    } else {
        printId = it->second;
    }
    ss << std::setw(1+(int)(log10(nextIndex))) << std::setfill (' ') << printId;
...
private:
    ...
    void determineFormats(unsigned int level_, const std::string& originalFormat) {
        ...
        setValue(level_, origFormatCopy, logFormatMap_);
    }
...
public:
    ...
    typedef boost::function<void(unsigned int level, const std::string &)> DispatchCallack;
    typedef std::map<std::string, DispatchCallack> DispatchCallbackMap;

    void registerDispatchCallback(const std::string& name, DispatchCallack callback) {
#if _ELPP_ENABLE_MUTEX
        internal::threading::ScopedLock slock_(mutex_);
        __EASYLOGGINGPP_SUPPRESS_UNSED(slock_);
#endif // _ELPP_ENABLE_MUTEX
        dispatchCallbacks_[name] = callback;
    }

    void unregisterDispatchCallback(const std::string& name)
    {
#if _ELPP_ENABLE_MUTEX
        internal::threading::ScopedLock slock_(mutex_);
        __EASYLOGGINGPP_SUPPRESS_UNSED(slock_);
#endif // _ELPP_ENABLE_MUTEX
        DispatchCallbackMap::iterator it = dispatchCallbacks_.find(name);
        if (it != dispatchCallbacks_.end()) {
            dispatchCallbacks_.erase(it);
        }
    }
...
private:
    ...
    bool configured_;
    internal::threading::Mutex mutex_;
    DispatchCallbackMap dispatchCallbacks_;
...
private:
    ...
    void log(void) {
        if (logger_->stream_) {
            for (Logger::DispatchCallbackMap::iterator it = logger_->dispatchCallbacks_.begin(); it != logger_->dispatchCallbacks_.end(); ++it) {
                it->second(severity_, currLine_);
            }
            if (logger_->typedConfigurations_->toStandardOutput(severity_)) {
                std::cout << currLine_ << std::endl;
            }
            currLine_ += "\n";
